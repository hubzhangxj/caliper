...
def concurrent_run(num):
    flag = 0
    try:
        dic = get_sections()
        for device in dic:
            run_test = run_case_thread(device, dic[device], num)
            run_test.start()
            run_test.join()
    except:
        flag =1
...
class run_case_thread(threading.Thread):
    def __init__(self, host, sections, num):
        ...
        for section in sections_run:
            ...
            subprocess.call("echo '$$ %s RUN DURATION %s Seconds'>>%s"
                            % (section,
                               ...
                               Folder.caliper_run_log_file), shell=True)
            subprocess.call(
                "echo '======================================================'>>%s" % (Folder.caliper_run_log_file),
                shell=True)
        ...
        try:
            ...
            try:
                ...
                result = subprocess.call(
                    ...
                    (os.path.join(caliper_path.config_files.config_dir, 'hosts'), cmd_sec_name, self.host, Folder.caliper_run_log_file), stdout=subprocess.PIPE, shell=True)
        ...
        try:
            ...
            if test_node != '':
                result = subprocess.call(
                    ...
                    (TEST_CASE_CONFIG, commands,test_node, getpass.getuser(), Folder.caliper_run_log_file), stdout=subprocess.PIPE,
                    shell=True)
            else:
                result = subprocess.call(
                    'ansible-playbook -i %s %s.yml --extra-vars "hosts=Device" -u %s>> %s 2>&1' %
                    (TEST_CASE_CONFIG, commands, getpass.getuser(), Folder.caliper_run_log_file),
